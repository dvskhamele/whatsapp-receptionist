...
create table if not exists public.tenants (
  ...
  plan text not null default 'trial'
    check (plan in ('trial', 'starter', 'professional', 'agency')),
...
create table if not exists public.tenant_config (
  ...
  booking_min_lead_minutes integer not null default 120
    check (booking_min_lead_minutes >= 0),
  booking_slot_step_minutes integer not null default 15
    check (booking_slot_step_minutes > 0),
...
create table if not exists public.conversations (
  ...
  channel text not null
    check (channel in ('whatsapp', 'instagram_dm', 'web_chat', 'sms')),
  customer_identifier text not null,
  customer_name text,
  status text not null default 'active'
    check (status in ('active', 'escalated', 'closed', 'spam')),
...
create table if not exists public.messages (
  ...
  message_type text not null default 'text'
    check (message_type in ('text', 'image', 'audio', 'document', 'location', 'status')),
...
create table if not exists public.appointments (
  ...
  -- Explicit end time avoids non-immutable expressions
  ...
  calendar_provider text
    check (
      ...
      or calendar_provider in ('google_calendar')
    ),
  ...
  constraint appointments_ends_after_start
    check (
      ends_at is null
      or ends_at > scheduled_at
    )
);
...
create table if not exists public.invoices (
  ...
  status text not null default 'draft'
    check (status in ('draft', 'sent', 'paid', 'void', 'failed')),
  sdi_status text
    check (
      sdi_status in (
        'not_required',
...
-- =========================================================
-- APPOINTMENT END TIME MIGRATION
-- =========================================================

-- If this script is being run against an existing database,
-- add the column first.
alter table public.appointments
  add column if not exists ends_at timestamptz;

-- Populate ends_at for existing appointments.
update public.appointments
set ends_at = scheduled_at + (duration_minutes * interval '1 minute')
where ends_at is null;

-- Prevent invalid appointment ranges.
do $$
begin
  if not exists (
    select 1
...
-- Existing rows must have an end time before the column becomes required.
alter table public.appointments
...
create index if not exists business_hours_tenant_id_idx
  on public.business_hours(tenant_id);
...
create index if not exists appointments_reminder_due_idx
  ...
  where status = 'confirmed'
    and (
      reminder_24h_queued_at is null
      or reminder_1h_queued_at is null
    );

create index if not exists appointments_ends_at_idx
  on public.appointments(tenant_id, ends_at);

create index if not exists knowledge_base_tenant_idx
...
create index if not exists opt_outs_tenant_idx
  on public.opt_outs(tenant_id);

create index if not exists usage_metrics_tenant_month_idx
  on public.usage_metrics(tenant_id, metric_month);

create index if not exists invoices_tenant_idx
  on public.invoices(tenant_id);

create index if not exists ai_prompts_tenant_key_idx
  on public.ai_prompts(tenant_id, prompt_key);

create index if not exists voice_events_tenant_created_idx
  on public.voice_events(tenant_id, created_at desc);

create index if not exists webhook_events_tenant_received_idx
  on public.webhook_events(tenant_id, received_at desc);

create index if not exists webhook_events_provider_external_idx
  on public.webhook_events(provider, external_id);

create index if not exists whatsapp_outbox_jobs_tenant_status_idx
  on public.whatsapp_outbox_jobs(tenant_id, status);
...
create index if not exists whatsapp_voice_jobs_tenant_status_idx
  on public.whatsapp_voice_jobs(tenant_id, status);
...
revoke execute
on function public.increment_usage_metrics(
  uuid,
  date,
  integer,
  integer,
...
as $$
  ...
  where kb.tenant_id = p_tenant_id
    ...
    and 1 - (kb.embedding <=> p_query_embedding)
        >= p_min_similarity
...
revoke execute
on function public.match_knowledge_base(
  uuid,
  vector(1536),
  integer,
  double precision
  ...
  with candidates as (
    select
      ...
      conversation.last_message_at
        + interval '24 hours'
        as customer_service_window_expires_at
    ...
    where job.status in ('pending', 'retry')
      ...
      and (
        ...
        or job.locked_at <
          now() - make_interval(
            secs => greatest(p_lock_ttl_seconds, 1)
          )
      )
...
revoke execute
on function public.claim_whatsapp_outbox_jobs(
  integer,
  text,
  integer
)
...
revoke execute
on function public.complete_whatsapp_outbox_job(
  uuid,
  text,
  jsonb,
  jsonb
...
begin
  if p_status not in (
    ...
    raise exception
      'invalid_whatsapp_outbox_failure_status';
  end if;
...
revoke execute
on function public.fail_whatsapp_outbox_job(
  uuid,
  text,
  timestamptz,
  text,
...
revoke execute
on function public.claim_whatsapp_voice_jobs(
  integer,
  text,
  integer
)
...
create policy tenants_owner_update_own
on public.tenants
for update
using (
  id = public.current_tenant_id()
  and public.current_tenant_role() = 'owner'
)
with check (
  id = public.current_tenant_id()
  and public.current_tenant_role() = 'owner'
...
create policy tenant_config_tenant_all
...
create policy business_hours_tenant_all
...
create policy knowledge_base_tenant_all
on public.knowledge_base
for all
using (
  tenant_id = public.current_tenant_id()
...
create policy integrations_tenant_all
...
create policy opt_outs_tenant_all
on public.opt_outs
for all
using (
  tenant_id = public.current_tenant_id()
...
create policy usage_metrics_tenant_select
...
create policy ai_prompts_tenant_select
...
create policy voice_events_admin_select
...
create policy whatsapp_outbox_jobs_admin_select
on public.whatsapp_outbox_jobs
for select
using (
  tenant_id = public.current_tenant_id()
  and public.current_tenant_role() in ('owner', 'admin')
...
create policy whatsapp_voice_jobs_admin_select
on public.whatsapp_voice_jobs
for select
using (
  tenant_id = public.current_tenant_id()
  and public.current_tenant_role() in ('owner', 'admin')
